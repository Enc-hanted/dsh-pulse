	...
	factory: (require) => {
		...
		function BucketChart({ buckets, granularity, today, modelBuckets, accentMap, byModel, names, legendRows, onDrillChange, onDrillHourly, t }) {
			...
			if (drillIdx !== null && rows[drillIdx] !== undefined) {
				...
				// modes. Clicks on the frame's empty space exit the drill.
				return jsxs("div", { children: [
					jsxs("div", { className: "dp_chartOuter", onClick: exitDrill, children: [
						jsxs("div", { className: "dp_drillHead", children: [
							...
							// 0.3% layer stays readable (its layer is too thin for
							// inline text).
						...
						jsxs("div", { className: "dp_drillStage", children: [
							jsx("div", { className: "dp_drillBar", style: drillStyle, children: segs.map((seg) => {
								...
								return jsx("div", {
									...
									onClick: (e) => {
										// A click pins the segment's split on screen; the
										// same click again unpins (hover keeps previewing).
										e.stopPropagation();
										setPinnedSeg((prev) => (prev === seg.key ? null : seg.key));
									},
									children: pct >= 14 ? `${Math.round(pct)}%` : null,
								}, seg.key);
							}) }),
							// In-place second-level drill: the pinned segment grows
							// inside the stage — anchored at the segment's exact
							// span — into its own token-type stack (input / cache
							// read / output), instead of a bar appearing below.
							byModel && pinnedSeg !== null && (() => {
								let left = 0;
								let width = 0;
								for (let k = 0; k < segs.length; k++) {
									const w = dayTotal > 0 ? (segs[k].tokens / dayTotal) * 100 : 0;
									if (segs[k].key === pinnedSeg) { width = w; break; }
									left += w;
								}
								if (width <= 0) return null;
								const seg = segs.find((s) => s.key === pinnedSeg);
								if (seg === undefined) return null;
								const tk = seg.tk ?? null;
								const parts = [
									{ key: "in", v: (tk?.input || 0) + (tk?.cacheWrite || 0), fill: "var(--dsw-alias-state-business-primary)" },
									{ key: "cache", v: tk?.cacheRead || 0, fill: "color-mix(in srgb,var(--dsw-alias-state-business-primary) 60%,var(--dsw-alias-state-business-tertiary))" },
									{ key: "out", v: tk?.output || 0, fill: "var(--dsw-alias-state-success-primary)" },
								];
								const sum = parts.reduce((s, p) => s + p.v, 0);
								const segLabel = seg.key === "__other__" || seg.bare === "" ? t("stackOther") : names.labelOf(seg.provider ?? "", seg.bare);
								return jsxs("div", {
									className: "dp_segStack",
									style: { left: `${left}%`, width: `${width}%` },
									title: segLabel,
									onClick: (e) => { e.stopPropagation(); setPinnedSeg(null); },
									children: parts.map((p) => {
										const share = sum > 0 ? (p.v / sum) * 100 : 0;
										return jsx("i", {
											style: { height: `${share}%`, background: p.fill },
											children: share >= 14 ? `${Math.round(share)}%` : null,
										}, p.key);
									}),
								}, pinnedSeg);
							})(),
		...
		function CostTrendPanel({ costs, today, actualByDay, accentMap, byModel, names, onDrillChange, onExitFocus, t }) {
			const [hover, setHover] = useState(null);
			const [drillIdx, setDrillIdx] = useState(null);
			const [seed, setSeed] = useState(null);
			const outerRef = useRef(null);
				...
				return jsxs("div", { children: [
					jsxs("div", { className: "dp_chartOuter", onClick: exitDrill, children: [
						jsxs("div", { className: "dp_drillHead", children: [
							jsx("button", { type: "button", className: "dp_setLink", onClick: exitDrill, children: `← ${t("backToDays")}` }),
							jsxs("span", { className: "dp_drillTotal", children: [
								jsx("b", { children: row.key === todayKey ? `${row.key} · ${t("today")}` : row.key }),
			...
			return jsxs("div", { children: [
				jsx("div", { ref: outerRef, className: "dp_chartOuter", children: jsxs("div", { className: "dp_chartGrid", children: [
					...
					rows.map((row, i) => {
						...
						return jsx("div", {
							className: `dp_col${isOffPeakDay(row.key) ? " dp_colOff" : ""}${sum === 0 ? " dp_colZero" : ""}`,
							onMouseEnter: () => setHover(i),
							onMouseLeave: () => setHover(null),
							onClick: (e) => enterDrill(i, e),
							children: [
								...
								jsxs("div", {
									key: "bar",
									className: `dp_bar${row.key === todayKey ? " dp_barToday" : ""}${sum > 0 ? " dp_barNonzero" : ""}`,
									style: { height: `${height}%` },
									children: stack.map((seg) => jsx("div", {
										className: seg.cls,
		 ...
		 *  balance condensed to a card (total + runway). It swapped places
		 *  with the window cost estimate — the estimate now lives inside the
		 *  expanded view. With `onToggle` the card opens the swap container's
		 *  balance view (same expand-focus grammar as the cost trend);
		 *  error / not-ok states stay readable with an inline retry. */
		function BalanceChip({ balance, series, active = false, onToggle, t }) {
			...
			return jsxs("div", {
				...
				jsxs("div", { className: "dp_sparkHead", children: [
					...
					ok && runway !== null && jsx("span", { className: "dp_sparkCount", children: runway < 3 ? fill(t("runwayLow"), { n: runway }) : fill(t("runwayDays"), { n: runway }) }),
					clickable && jsx("span", { className: "dp_sparkChevron", "aria-hidden": true, children: "▾" }),
				] }),
				ok ? jsxs("div", { className: "dp_balanceChipRow", children: [
					jsx("i", { className: `dp_balanceDot${data.isAvailable === true ? "" : " dp_balanceDotOff"}` }),
		...
		function PulseDashboard({ t, headerExtra, onConfigure, floatActions = false, renderSlot: renderPanelsSlot, actions }) {
			...
			/** 余额·预算聚焦态：与费用走势同一套展开聚焦语法，三个视图互斥。 */
			const [balanceFocus, setBalanceFocus] = useState(false);
			const toggleCostView = () => { setCostFocus((v) => !v); setBalanceFocus(false); };
			const toggleBalanceView = () => { setBalanceFocus((v) => !v); setCostFocus(false); };
			...
			const focusOn = trendFocused || costFocusActive || balanceFocusActive;
			/** Escape leaves the balance view (the cost trend unwinds its own:
			 *  day drill first, then the takeover). */
			useEffect(() => {
				if (!balanceFocusActive) return undefined;
				const onKey = (e) => { if (e.key === "Escape") setBalanceFocus(false); };
				window.addEventListener("keydown", onKey);
				return () => window.removeEventListener("keydown", onKey);
			}, [balanceFocusActive]);
			const showChips = panels.chips !== false || (panels.cost !== false && costEnabled) || balanceCardOn;
			return jsxs("div", { className: `dp_root${focusOn ? " dp_focusOn" : ""}`, "data-dp-theme": theme, children: [
				...
				showChips && jsxs("div", { className: "dp_chips", children: [
					 ...
					 *  余额不可用/第三方/项目详情时回退为费用估算 chip。 */
					!balanceCardOn && panels.cost !== false && costEnabled && !hideCostish && jsxs("div", { className: "dp_chip", children: [
				 ...
				 *  估算占位对换后也在这里展开。 */
				panels.trend !== false && jsxs("div", { className: focusOn ? "dp_focusKeep" : undefined, children: [
					jsxs("div", { key: balanceFocusActive ? "balance" : costFocusActive ? "cost" : "use", className: "dp_swapIn", children: [
						jsxs("div", { className: `dp_panelTitle${costFocusActive || (!balanceFocusActive && !hourly && !heatmap && hasModelSplit) ? " dp_hasStack" : ""}`, children: [
							jsx("span", { children: balanceFocusActive ? t("balanceTitle") : costFocusActive ? t("costTrend") : t("trendTitle") }),
							!balanceFocusActive && jsx("span", { className: "dp_panelCount", children: !costFocusActive && hourly ? t("hourCount") : fill(t("dailyCount"), { n: bucketCount }) }),
							costFocusActive ? jsxs("div", { className: "dp_seg dp_stackToggle", children: [
							...
							] }) : (!balanceFocusActive && !hourly && !heatmap && hasModelSplit && jsxs("div", { className: "dp_seg dp_stackToggle", children: [
						...
						jsx("div", { className: "dp_trendBody", children: balanceFocusActive
							...
							: hourly
								...
								: heatmap
									...
									: jsxs("div", { children: [
										jsx(BucketChart, { buckets: view.buckets, granularity: "day", today: data.today, modelBuckets: view.modelBuckets, accentMap, byModel, names, legendRows: modelLegendRows, onDrillChange: setTrendDrilled, onDrillHourly: () => setRangeKey("1"), t }),
										!trendFocused && jsx(BucketXLabels, { buckets: view.buckets, granularity: "day", today: data.today, t }),
									] }) }),
					] }),
					focusOn && jsx("div", { className: "dp_focusNote", children: t("focusNote") }),
				] }),
