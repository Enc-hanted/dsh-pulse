	...
	factory: (require) => {
		...
		function BucketChart({ buckets, granularity, today, modelBuckets, accentMap, byModel, names, legendRows, onDrillChange, onDrillHourly, t }) {
			...
			if (drillIdx !== null && rows[drillIdx] !== undefined) {
				...
				return jsxs("div", { className: "dp_drill", onClick: exitDrill, children: [
					...
					jsxs("div", { className: "dp_drillBarWrap", children: [
						...
						byModel && subSeg !== null && jsxs("div", { className: "dp_drillSub", children: [
							jsx("div", { className: "dp_drillSubNote", children: `${subLabel} · ${t("legendInput")} ${fmtPct(subIn)} (${fmtTokens(subIn)}) · ${t("legendCache")} ${fmtPct(subCache)} (${fmtTokens(subCache)}) · ${t("legendOutput")} ${fmtPct(subOut)} (${fmtTokens(subOut)})` }),
							jsxs("div", { className: "dp_drillSubBar", children: [
								jsx("div", { className: "dp_drillSubSeg", style: { width: `${subSum > 0 ? (subIn / subSum) * 100 : 0}%`, background: "var(--dsw-alias-state-business-primary)" } }),
								jsx("div", { className: "dp_drillSubSeg", style: { width: `${subSum > 0 ? (subCache / subSum) * 100 : 0}%`, background: "color-mix(in srgb,var(--dsw-alias-state-business-primary) 60%,var(--dsw-alias-state-business-tertiary))" } }),
								jsx("div", { className: "dp_drillSubSeg", style: { width: `${subSum > 0 ? (subOut / subSum) * 100 : 0}%`, background: "var(--dsw-alias-state-success-primary)" } }),
							] }),
						] }),
		...
		function CostTrendPanel({ costs, today, actualByDay, accentMap, byModel, names, onDrillChange, onExitFocus, t }) {
			...
			if (drillIdx !== null && rows[drillIdx] !== undefined) {
				...
				return jsxs("div", { className: "dp_drill", onClick: exitDrill, children: [
					...
					jsx("div", { className: "dp_drillBarWrap", children: jsx("div", { className: "dp_drillBar", style: drillStyle, children: segs.map((seg) => {
						const pct = dayTotal > 0 ? (seg.tokens / dayTotal) * 100 : 0;
						return jsx("div", {
		...
		function PulseDashboard({ t, headerExtra, onConfigure, floatActions = false, renderSlot: renderPanelsSlot, actions }) {
			...
			const focusOn = trendFocused || costFocusActive;
			...
			return jsxs("div", { className: `dp_root${focusOn ? " dp_focusOn" : ""}`, "data-dp-theme": theme, children: [
				...
				showChips && jsxs("div", { className: "dp_chips", children: [
					...
					panels.cost !== false && costEnabled && cost.configured === true && costDays !== null
						...
						&& jsx(CostSpark, {
							...
							onToggle: costFocusable ? () => setCostFocus((v) => !v) : undefined,
							t,
						}),
				...
				panels.balance !== false && !thirdParty && project === "" && jsx(BalanceBar, { balance, series: Array.isArray(data?.balanceSeries) ? data.balanceSeries : null, t }),
				panels.budget !== false && costEnabled && !hideCostish && project === "" && jsx(BudgetCard, { t }),
				/** 用量走势 / 费用走势共享同一个槽位：点击 spark 卡片后，费用
				 *  走势在此向下展开替换用量走势（key 切换重放展开动画）；聚焦
				 *  态下本节保持高亮（dp_focusKeep），其余面板压暗，底部附官网
				 *  口径小字。再点 spark 卡片或 Esc 交还用量走势。 */
				panels.trend !== false && jsxs("div", { className: focusOn ? "dp_focusKeep" : undefined, children: [
					jsxs("div", { key: costFocusActive ? "cost" : "use", className: "dp_swapIn", children: [
						jsxs("div", { className: `dp_panelTitle${costFocusActive || (!hourly && !heatmap && hasModelSplit) ? " dp_hasStack" : ""}`, children: [
							jsx("span", { children: costFocusActive ? t("costTrend") : t("trendTitle") }),
							jsx("span", { className: "dp_panelCount", children: !costFocusActive && hourly ? t("hourCount") : fill(t("dailyCount"), { n: bucketCount }) }),
						...
						jsx("div", { className: "dp_trendBody", children: costFocusActive
							? jsxs("div", { children: [
								jsx(CostTrendPanel, {
