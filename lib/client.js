window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		backToUsage: "Back to usage trend",
		blankToReturn: "Click blank area to go back",
		weekCount: "{n} weeks",
		monthCount: "{n} months",
		chipActivity: "Sessions · Turns",
		chipTools: "{n} tool calls",
		 ...
		 *  empty window). Short enough to look immediate, long enough that a
		 *  second agent turn has landed. */
		...
		function scheduleEmptyRetry(from, to) {
			...
			statsRetry = setTimeout(() => {
				...
				if (data !== null && Array.isArray(data.sessions) && data.sessions.length > 0) return;
		...
		function BucketChart({ buckets, granularity, today, modelBuckets, accentMap, byModel, names, legendRows, onDrillChange, onDrillHourly, t }) {
		...
		function CostTrendPanel({ costs, today, actualByDay, accentMap, byModel, names, bucket = "day", onDrillChange, onExitFocus, t }) {
			const [hover, setHover] = useState(null);
			const [drillIdx, setDrillIdx] = useState(null);
			const [seed, setSeed] = useState(null);
			const outerRef = useRef(null);
			...
			/** Escape unwinds one level at a time: first the drilled day, then
			 *  the cost-trend takeover itself (handed back to the dashboard,
			 *  which restores the usage trend). */
			const exitDrill = () => { setDrillIdx(null); setHover(null); onDrillChange?.(null); };
			useEffect(() => {
				const onKey = (e) => {
					if (e.key !== "Escape") return;
					if (drillIdx === null) onExitFocus?.();
					else exitDrill();
				};
				window.addEventListener("keydown", onKey);
				return () => window.removeEventListener("keydown", onKey);
			}, [drillIdx, onExitFocus]);
		...
		/** Shared expand chevron for the launcher mid cards (a drawn SVG —
		 *  glyph/emoji arrows render inconsistently and look cheap). */
		const chevronSvg = jsxs("svg", { width: 13, height: 13, viewBox: "0 0 16 16", fill: "none", "aria-hidden": true, children: [
			jsx("path", { d: "M3.5 6l4.5 4.5L12.5 6", stroke: "currentColor", strokeWidth: 1.6, strokeLinecap: "round", strokeLinejoin: "round" }),
		] });

		/** Chip-row launcher for the balance & budget view: the official
		 *  balance condensed to a mid card — total, runway, updated-at and a
		 *  refresh button all live here (the expanded view does NOT repeat
		 *  the balance bar). Swapped places with the window cost estimate,
		 *  which now lives inside the expanded view. With `onToggle` the card
		 *  opens the swap container's balance view (same expand-focus grammar
		 *  as the cost trend); error / not-ok states stay readable. */
		function BalanceChip({ balance, series, active = false, onToggle, t }) {
			const data = balance.data;
			const clickable = typeof onToggle === "function";
			const ok = data !== null && data.configured === true && data.ok === true;
			const runway = ok ? balanceRunwayDays(data, series) : null;
			const failErr = data !== null && data.configured === true ? (data.error ?? "?") : (balance.error ?? "?");
			return jsxs("div", {
				className: `dp_chip${clickable ? " dp_sparkBtn" : ""}${active ? " dp_sparkActive dp_focusKeep" : ""}`,
				...
				jsxs("div", { className: "dp_sparkHead", children: [
					jsx("span", { className: "dp_chipLabel", children: t("balanceTitle") }),
					clickable && jsx("span", { className: "dp_sparkChevron", "aria-hidden": true, children: chevronSvg }),
				] }),
				ok ? [
					jsxs("div", { className: "dp_balanceChipRow", key: "val", children: [
						jsx("i", { className: `dp_balanceDot${data.isAvailable === true ? "" : " dp_balanceDotOff"}` }),
						jsx("span", { className: "dp_chipValue", children: `${data.currency === "CNY" ? "¥" : ""}${fmtCost(data.total ?? 0)}${data.currency !== "CNY" ? ` ${data.currency}` : ""}` }),
						data.isAvailable !== true && jsx("span", { className: "dp_balanceWarn", children: t("balanceUnavailable") }),
					] }),
					jsxs("div", { className: "dp_balanceChipRow", key: "foot", children: [
						runway !== null && jsx("span", { className: "dp_chipNote", children: runway < 3 ? fill(t("runwayLow"), { n: runway }) : fill(t("runwayDays"), { n: runway }) }),
						jsxs("span", { className: "dp_balanceTime", children: [
							fill(t("balanceAt"), { t: fmtClock(data.fetchedAt) }),
							jsx("button", {
				...
				] : jsx("div", { className: "dp_balanceChipRow", children: [
					jsx("i", { className: "dp_balanceDot dp_balanceDotOff" }),
					jsx("span", { className: "dp_chipNote", children: fill(t("balanceFailed"), { err: failErr }) }),
					jsx("button", {
						type: "button", className: "dp_miniBtn",
		...
		/** Merged sessions + turns mid card (the two small KPI chips are gone;
		 *  token/cache chips died with the CacheRing duplicate). Clicking opens
		 *  the project-ranking focus view of the swap container. */
		function SessionsCard({ totals, active = false, onToggle, t }) {
			const clickable = typeof onToggle === "function";
			return jsxs("div", {
				className: `dp_chip${clickable ? " dp_sparkBtn" : ""}${active ? " dp_sparkActive dp_focusKeep" : ""}`,
				onClick: clickable ? onToggle : undefined,
				...
				children: [
				jsxs("div", { className: "dp_sparkHead", children: [
					jsx("span", { className: "dp_chipLabel", children: t("chipActivity") }),
					clickable && jsx("span", { className: "dp_sparkChevron", "aria-hidden": true, children: chevronSvg }),
				] }),
				jsxs("div", { className: "dp_balanceChipRow", children: [
					jsx("span", { className: "dp_chipValue", children: `${totals.sessions || 0} / ${totals.turns || 0}` }),
				] }),
				jsxs("div", { className: "dp_balanceChipRow", children: [
					jsx("span", { className: "dp_chipNote", children: fill(t("chipTools"), { n: totals.toolCalls || 0 }) }),
		 ...
		 */
		function CostSpark({ days, actual, active = false, onToggle, t }) {
			...
			/** Interactive chip: with `onToggle` the spark card becomes the
			 *  switch for the cost-trend takeover of the usage-trend slot
			 *  (`active` mirrors that state; `dp_focusKeep` keeps the chip
			 *  lit while every other panel is dimmed). Without it the chip
			 *  stays a passive read-only summary. */
			const clickable = typeof onToggle === "function";
			return jsxs("div", {
				className: `dp_chip dp_sparkCard${clickable ? " dp_sparkBtn" : ""}${active ? " dp_sparkActive dp_focusKeep" : ""}`,
				onClick: clickable ? onToggle : undefined,
				role: clickable ? "button" : undefined,
				tabIndex: clickable ? 0 : undefined,
				"aria-pressed": clickable ? active : undefined,
				...
				jsxs("div", { className: "dp_sparkHead", children: [
					...
					jsx("span", { className: "dp_sparkCount", children: `${fill(t("dailyCount"), { n })} · ${fill(t("costSum"), { v: `¥${fmtCost(totals.reduce((s, v) => s + v, 0))}` })}` }),
					clickable && jsx("span", { className: "dp_sparkChevron", "aria-hidden": true, children: chevronSvg }),
				] }),
		...
		function PulseDashboard({ t, headerExtra, onConfigure, floatActions = false, renderSlot: renderPanelsSlot, actions }) {
			...
			/** 余额·预算聚焦态：与费用走势同一套展开聚焦语法，视图互斥。 */
			const [balanceFocus, setBalanceFocus] = useState(false);
			/** 项目排行聚焦态（会话·回合卡触发）；项目排行退出默认页。 */
			const [projectsFocus, setProjectsFocus] = useState(false);
			/** 点击压暗区退出聚焦时，用 remount 代偿收起图内已打开的 drill。 */
			const [useDrillEpoch, setUseDrillEpoch] = useState(0);
			const [costDrillEpoch, setCostDrillEpoch] = useState(0);
			const toggleCostView = () => { setCostFocus((v) => !v); setBalanceFocus(false); setProjectsFocus(false); };
			const toggleBalanceView = () => { setBalanceFocus((v) => !v); setCostFocus(false); setProjectsFocus(false); };
			const toggleProjectsView = () => { setProjectsFocus((v) => !v); setCostFocus(false); setBalanceFocus(false); };
			...
			/** Cost-trend takeover availability: needs the usage-trend slot to
			 *  swap into, a multi-day window, configured pricing and the spark
			 *  chip's own gates. When any gate drops (e.g. the range narrows
			 *  to today) the usage trend quietly takes the slot back. */
			...
			const costFocusActive = costFocus && costFocusable;
			/** Balance chip ⇄ cost-estimate chip swapped places: the balance
			 *  card occupies the chip-row slot whenever the official balance
			 *  is relevant (official account context, resolved once) — the
			 *  window estimate then lives inside the expanded view. */
			/** Resolved = the balance endpoint answered meaningfully: an ok
			 *  payload, or a failure worth showing (with retry). A payload
			 *  with the feature unconfigured stays silent, like BalanceBar. */
			const balanceResolved = balance.error !== null
				|| (balance.data !== null && balance.data.configured === true);
			const balanceCardOn = panels.balance !== false && !thirdParty && project === "" && balanceResolved;
			const balanceFocusable = balanceCardOn && panels.trend !== false;
			const balanceFocusActive = balanceFocus && balanceFocusable;
			/** 项目排行聚焦：会话·回合卡触发，仅全局视图（非项目详情）可用。 */
			const projectsFocusable = panels.trend !== false && panels.projects !== false && project === "";
			const projectsFocusActive = projectsFocus && projectsFocusable;
			...
			const focusOn = trendFocused || costFocusActive || balanceFocusActive || projectsFocusActive;
			const swapView = projectsFocusActive ? "projects" : balanceFocusActive ? "balance" : costFocusActive ? "cost" : "use";
			/** Escape leaves the balance / projects views (the cost trend
			 *  unwinds its own: day drill first, then the takeover). */
			useEffect(() => {
				if (!balanceFocusActive && !projectsFocusActive) return undefined;
				const onKey = (e) => {
					if (e.key !== "Escape") return;
				...
				window.addEventListener("keydown", onKey);
				return () => window.removeEventListener("keydown", onKey);
			}, [balanceFocusActive, projectsFocusActive]);
			/** Clicking dimmed space (anything outside the focused section and
			 *  the launcher grid) exits the focus; open drills are reset via a
			 *  remount epoch since their state lives inside the charts. */
			const exitFocusAll = () => {
				setCostFocus(false);
				setBalanceFocus(false);
				setProjectsFocus(false);
				setTrendDrilled(false);
				setCostDrilled(false);
				if (trendFocused) setUseDrillEpoch((v) => v + 1);
				if (costDrillOn) setCostDrillEpoch((v) => v + 1);
			};
			const onRootClick = focusOn ? (e) => {
				const el = e.target;
				if (el instanceof Element && (el.closest(".dp_focusKeep") !== null || el.closest(".dp_chips") !== null)) return;
				exitFocusAll();
			} : undefined;
			/** Bring the focused container to the vertical center of the
			 *  overlay's visible area when a focus begins. */
			const swapRef = useRef(null);
			const prevFocusRef = useRef(false);
			useEffect(() => {
				if (focusOn && !prevFocusRef.current) {
					swapRef.current?.scrollIntoView?.({ behavior: "smooth", block: "center" });
				}
				prevFocusRef.current = focusOn;
			}, [focusOn]);
			...
			const showChips = panels.chips !== false || panels.cache !== false || balanceCardOn || (panels.cost !== false && costEnabled);
			return jsxs("div", { className: `dp_root${focusOn ? " dp_focusOn" : ""}`, "data-dp-theme": theme, children: [
				header,
				jsx(Toolbar, { rangeKey, setRangeKey, custom, setCustom, project, setProject, knownProjects: view.knownProjects, model, setModel, knownModels: view.knownModels, names, t, filterSlot: renderPanelsSlot }),
				showChips && jsxs("div", { className: "dp_chips", children: [
					panels.cache !== false && jsx(CacheRing, { totals, t }),
					panels.cost !== false && cost.configured === true && costDays !== null
						&& costDays.some((d) => (d.peak || 0) + (d.offpeak || 0) > 0)
						&& jsx(CostSpark, {
							days: costDays,
							actual: thirdParty ? null : (Array.isArray(data?.balanceSeries) ? data.balanceSeries : null),
							active: costFocusActive,
							onToggle: costFocusable ? toggleCostView : undefined,
							t,
						}),
					/** 占位对换：官方余额卡在槽时费用估算让位（估算进展开视图）；
					 *  余额不可用/第三方/项目详情时回退为费用估算卡。 */
					balanceCardOn ? jsx(BalanceChip, {
						balance,
						series: Array.isArray(data?.balanceSeries) ? data.balanceSeries : null,
						active: balanceFocusActive,
						onToggle: balanceFocusable ? toggleBalanceView : undefined,
						t,
					}) : (panels.cost !== false && costEnabled && !hideCostish && jsxs("div", { className: "dp_chip", children: [
				...
				panels.chips !== false && jsx(SessionsCard, {
					totals,
					active: projectsFocusActive,
					onToggle: projectsFocusable ? toggleProjectsView : undefined,
					t,
				}),
				...
				// A selected project switches the dashboard to its detail view:
				// the global balance / budget / project ranking step back and
				// the project's sessions take over (the session panel renders
				// only that project's rows).
				/** 通用切换容器：用量走势（默认常驻）⇄ 费用走势（spark 卡片）⇄
				 *  余额·预算（官方余额卡片）。点击对应卡片向下展开替换（key 切
				 *  换重放展开动画），聚焦态下本节保持高亮（dp_focusKeep），其余
				 *  面板压暗，底部附官网口径小字；再点同一卡片或 Esc 交还用量走
				 *  势。原「官方余额 / 月度预算」全宽节并入余额·预算视图，费用
				 *  估算占位对换后也在这里展开。 */
				/** 通用切换容器（四态互斥）：用量走势（默认常驻）⇄ 费用走势
				 *  （spark 卡）⇄ 余额·预算（官方余额卡）⇄ 项目排行（会话·回合
				 *  卡）。点击对应卡片向下展开替换（key 切换重放过渡），容器体
				 *  各视图等高（256px），聚焦态下本节保持高亮（dp_focusKeep）并
				 *  滚动到可视区中央，其余面板压暗；点压暗空白、右上角返回按钮
				 *  或 Esc 逐层交还。 */
				panels.trend !== false && jsxs("div", { ref: swapRef, className: focusOn ? "dp_focusKeep" : undefined, children: [
					jsxs("div", { key: swapView, className: "dp_swapIn", children: [
						jsxs("div", { className: `dp_panelTitle${swapView === "cost" || (swapView === "use" && !hourly && !heatmap && hasModelSplit) ? " dp_hasStack" : ""}`, children: [
							jsx("span", { children: swapView === "projects" ? t("projectsTitle") : swapView === "balance" ? t("balanceTitle") : swapView === "cost" ? t("costTrend") : t("trendTitle") }),
							swapView !== "projects" && swapView !== "balance" && jsx("span", { className: "dp_panelCount", children: swapView === "cost"
							...
							/** 聚焦提示：写在 dp_seg dp_stackToggle 前的空位（无 toggle
							 *  的视图右对齐）。语义是"返回"而非"关闭"——点击压暗空白
							 *  逐层/整体交还用量走势。 */
							focusOn && jsx("span", { className: "dp_focusHint", style: swapView === "cost" || (swapView === "use" && !hourly && !heatmap && hasModelSplit) ? undefined : { marginLeft: "auto" }, children: t("blankToReturn") }),
							...
							] }) : (swapView === "use" && !hourly && !heatmap && hasModelSplit && jsxs("div", { className: "dp_seg dp_stackToggle", children: [
						...
						jsx("div", { className: "dp_trendBody", children: swapView === "projects"
							? jsx("div", { className: "dp_projScroll", children: jsx(ProjectTable, {
								projects: view.projects,
								topProjects: data.topProjects,
								onSelect: (p) => { setProject(p); setProjectsFocus(false); },
								t,
							}) })
							: swapView === "balance"
							? jsxs("div", { className: "dp_balanceView", children: [
								panels.cost !== false && costEnabled && !hideCostish && jsxs("div", { className: "dp_estCard", children: [
									jsxs("div", { className: "dp_sparkHead", children: [
										jsx("span", { className: "dp_chipLabel", children: t("chipCost") }),
										jsx("span", { className: "dp_sparkCount", children: fill(t("dailyCount"), { n: bucketCount }) }),
									] }),
									cost.configured === true
										? jsx("span", { className: "dp_chipValue dp_costOk", children: `${fmtCost(cost.total ?? 0)} CNY` })
										: (onConfigure !== undefined
								...
								jsxs("div", { className: "dp_balanceGrow", children: [
									panels.budget !== false && costEnabled && !hideCostish && jsx(BudgetCard, { t }),
								] }),
							] })
							: swapView === "cost"
							? jsxs("div", { children: [
								jsx(CostTrendPanel, {
									...
									onExitFocus: () => setCostFocus(false),
									t,
								}),
								!costDrillOn && jsx(BucketXLabels, { buckets: view.buckets, granularity: "day", today: data.today, t }),
							] })
							: hourly
								...
								: heatmap
									...
									: jsxs("div", { children: [
										jsx(BucketChart, { key: useDrillEpoch, buckets: view.buckets, granularity: "day", today: data.today, modelBuckets: view.modelBuckets, accentMap, byModel, names, legendRows: modelLegendRows, onDrillChange: setTrendDrilled, onDrillHourly: () => setRangeKey("1"), t }),
										!trendFocused && jsx(BucketXLabels, { buckets: view.buckets, granularity: "day", today: data.today, t }),
									] }) }),
					] }),
					focusOn && swapView !== "projects" && jsx("div", { className: "dp_focusNote", children: t("focusNote") }),
				] }),
				...
				panels.sessions !== false && project !== "" && jsxs("div", { children: [
					jsx("div", { className: "dp_panelTitle", children: t("projectDetailTitle") }),
					jsx(SessionsPanel, {
