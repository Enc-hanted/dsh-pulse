window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 *  per-model accent colors (v0.5); clicking a column pins a per-day
		 *  breakdown card and dims the rest of the timeline. */
		function BucketChart({ buckets, granularity, today, modelBuckets, accentMap, byModel, names, t }) {
			const [hover, setHover] = useState(null);
			const [pinned, setPinned] = useState(null);
			const rows = Array.isArray(buckets) ? buckets : [];
			const matrices = Array.isArray(modelBuckets) ? modelBuckets : [];
			// A new window or dimension invalidates the pinned day.
			useEffect(() => setPinned(null), [buckets, byModel]);
			...
			const pinnedIdx = pinned !== null && rows[pinned] !== undefined ? pinned : null;
			...
			const segsFor = (i, bucket) => {
				...
				if (rest > 0) top.push({ key: "__other__", bare: "", provider: "", tokens: rest });
				return top.map((seg) => ({
					...seg,
			...
			return jsxs("div", { children: [
				jsx("div", { className: "dp_chartOuter", children: jsxs("div", { className: `dp_chartGrid${pinnedIdx !== null ? " dp_hasPin" : ""}`, children: [
					...
					rows.map((bucket, i) => {
						...
						return jsx("div", {
							className: `dp_col${pinnedIdx === i ? " dp_colPin" : ""}`,
							onMouseEnter: () => setHover(i),
							onMouseLeave: () => setHover(null),
							onClick: () => setPinned(pinnedIdx === i ? null : i),
							children: [
				...
				pinnedIdx !== null && (() => {
					const bucket = rows[pinnedIdx];
					const detail = byModel
						? segsFor(pinnedIdx, bucket)
							.map((seg) => ({
								key: seg.key,
								fill: seg.fill ?? otherFill,
								label: seg.key === "__other__" || seg.bare === "" ? t("stackOther") : names.labelOf(seg.provider ?? "", seg.bare),
								tokens: seg.tokens,
							}))
						: [
							{ key: "in", fill: "var(--dsw-alias-state-business-primary)", label: t("legendInput"), tokens: (bucket.input || 0) + (bucket.cacheWrite || 0) },
							{ key: "cache", fill: "color-mix(in srgb,var(--dsw-alias-state-business-primary) 60%,var(--dsw-alias-state-business-tertiary))", label: t("legendCache"), tokens: bucket.cacheRead || 0 },
							{ key: "out", fill: "var(--dsw-alias-state-success-primary)", label: t("legendOutput"), tokens: bucket.output || 0 },
						];
					return jsxs("div", { className: "dp_dayCard", children: [
						jsxs("div", { className: "dp_dayCardHead", children: [
							jsx("b", { children: bucket.key === todayKey ? `${bucket.key} ${t("today")}` : bucket.key }),
							jsx("button", { type: "button", className: "dp_setLink", onClick: () => setPinned(null), children: t("close") }),
						] }),
						detail.filter((row) => row.tokens > 0).map((row) => jsxs("div", { className: "dp_dayRow", children: [
							jsx("i", { className: "dp_dayDot", style: { background: row.fill } }),
							jsx("span", { className: "dp_dayName", title: row.label, children: row.label }),
							jsx("span", { className: "dp_dayVal", children: fmtTokens(row.tokens) }),
						] }, row.key)),
						detail.every((row) => row.tokens <= 0) && jsx("div", { className: "dp_dayRow", children: jsx("span", { className: "dp_dayName", children: t("cacheNa") }) }),
					] });
				})(),
			] });
		}
				...
				panels.trend !== false && jsxs("div", { children: [
					...
					jsx("div", { className: "dp_trendBody", children: hourly
						...
						: heatmap
							...
							: jsxs("div", { children: [
								jsx(BucketChart, { buckets: view.buckets, granularity: "day", today: data.today, modelBuckets: view.modelBuckets, accentMap, byModel, names, t }),
