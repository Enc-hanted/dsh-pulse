window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** The reactive store engine behind slot store seats (0.1.7+). Optional:
		 *  absent on older hosts, where every store-seat feature below degrades
		 *  to the render-site props it mirrors. */
		let defineStore = null;
		try { defineStore = require("@deepseek-ai/dsh-client-store").defineStore ?? null; } catch { /* old module table */ }
		/** 0.1.7 renamed the size-suffixed icon exports (`IconXOutline16` /
		 *  `IconXOutline14`) to stroke-weight variants (`IconXOutlineMedium`,
		 *  1.3px, same `size` prop; `IconXOutlineRegular` is 1px), removing the
		 *  legacy names outright — a bundle built before the rename renders
		 *  `undefined` for them and the host slot dies with React #130.
		 *  Reading the module through this proxy aliases each legacy name onto
		 *  its modern component, so one build serves both host generations;
		 *  an icon the running host provides under neither name degrades to a
		 *  no-op component instead of crashing the mounting slot. */
		...
		const noopIcon = () => null;
		const primitives = new Proxy(primitivesModule, {
			get(target, prop) {
		...
		if (typeof document !== "undefined") {
			...
			tag.textContent = css + themeCss + rowCss + chartCss;
		}
		//#endregion
		...
		const zh = {
			...
			windowEmptyBody: "已有其它会话记录，只是最近 {n} 天没有落在所选区间内的用量；换个范围或等本轮对话产生 token 后会自动更新。",
		...
		detailSectionBody: "跨会话 token 用量、缓存命中、项目分布与官方余额对账。仪表盘入口在左侧边栏（数据图标）或 /pulse 命令；定价规则可在本页直接配置。",
		...
		drillByHour: "按小时查看 →",
			...
			sessionSubTotal: "子代理共 {tokens} · {cost}",
			sessionCreated: "创建",
		...
		const en = {
			...
			windowEmptyBody: "Other sessions exist, but none landed in the last {n} days of the selected range. Pick another range, or let this conversation produce tokens and it refreshes itself.",
		...
		detailSectionBody: "Cross-session token usage, cache hit rate, project breakdown and official-balance reconciliation. Open the dashboard from the sidebar (the data icon) or the /pulse command; pricing rules are configurable right on this page.",
		...
		panelCostTrend: "Cost trend chart",
		legendPeak: "Peak",
		...
		resizeCards: "Drag to resize the card row (double-click to reset)",
		back: "Back",
		...
		drillByHour: "View by hour →",
			...
			sessionSubTotal: "subagents {tokens} · {cost}",
		...
		const OFFICIAL_PRICE_SCHEDULES = [
		  {
		    from: "2026-09-10",
		    rules: [
		 ...
		 */
		function buildView(sessions, { granularity = "day", from, to, project = "", model = "", pricing = [], fx = {}, monthly = [] }) {
		 ...
		 */
		function costSeries(sessions, { from, to, project = "", model = "", pricing = [], fx = {}, monthly = [] }) {
		 ...
		 */
		function sessionGroups(sessions, { pricing = [], fx = {}, monthly = [], model = "", catalog = null } = {}) {
		  const modelFilter = model === "" || model === null || model === undefined ? null : String(model);
		  const filterComposite = modelFilter !== null && modelFilter.includes(MODEL_SEP);
		 ...
		 */
		function breaksSegments(events, breaks, { pricing = [], fx = {}, monthly = [], model = "", catalog = null } = {}) {
		  const modelFilter = model === "" || model === null || model === undefined ? null : String(model);
		  const filterComposite = modelFilter !== null && modelFilter.includes(MODEL_SEP);
		 ...
		 *  polling forever. */
		const EMPTY_RETRY_FIRST_MS = 1200;
		const EMPTY_RETRY_BACKOFF_MS = 4000;
		const EMPTY_RETRY_MAX = 8;
		let statsEmptyAttempts = 0;
		...
		const BALANCE_EVENT = "pulse:balance";
		...
		let settingsCache = { at: 0, value: null, inflight: null };
		...
		function invalidateSettings() { settingsCache.value = null; settingsCache.at = 0; }
		//#endregion
		...
		/** First-row card height in px, set by dragging the row's corner grip.
		 *  `null` keeps the width-derived ratio; a number overrides it. Like
		 *  every other preference here it is browser-local, not host state. */
		const CARD_H_STORAGE = "dsh-pulse:card-h";
		const CARD_H_MIN = 140;
		const CARD_H_MAX = 380;
		function clampCardH(value) {
			if (value === null || value === undefined || value === "") return null;
			const n = Math.round(Number(value));
			if (!Number.isFinite(n)) return null;
			return Math.max(CARD_H_MIN, Math.min(CARD_H_MAX, n));
		}
		function loadCardH() {
			try { return clampCardH(localStorage.getItem(CARD_H_STORAGE)); } catch (error) { return null; }
		}
		function saveCardH(value) {
			try {
				if (value === null) localStorage.removeItem(CARD_H_STORAGE);
				else localStorage.setItem(CARD_H_STORAGE, String(Math.round(value)));
		...
		const TREND_STACK_STORAGE = "dsh-pulse:trend-stack";
		...
		const COST_STACK_STORAGE = "dsh-pulse:cost-stack";
		...
		const THEME_STORAGE = "dsh-pulse:theme";
		const THEMES = ["blue", "pink", "orange", "bw"];
		function loadTheme() {
			try {
		...
		function catalogNames(catalog, keys = []) {
			 ...
			 *  rules and events), else the bare id. */
			...
			const labelOf = (provider, model) => {
				...
				return `${providerName.get(p) || p} · ${name}`;
			};
			 ...
			 *  official same-id models read at a glance). */
		...
		//#region charts
		//#region model accents
		/** Per-model accent colors for the model charts (v0.5): vendor brand
		 *  colors first — matched on the model id so `gpt-4o` hits OpenAI's
		 *  tokens while `glm-5.3` never trips the short `o*` keys — then a
		 *  golden-angle hue spread with deterministic collision avoidance for
		 *  unknown vendors, and user overrides from the pricing page's
		 *  模型配色 card winning over everything. Same-vendor siblings share
		 *  one family spread between TWO anchors (`hex` → `soft`, hue and
		 *  lightness together — the official-gradient trick: MiniMax's brand
		 *  book pairs #FF3763→#FF7038 the same way), so every step stays
		 *  saturated and clearly distinct even at dot size: the heaviest
		 *  model wears the pure brand color, the lightest wears `soft`. */
		function accentHash(id) {
			let h = 0x811c9dc5;
		...
		/** The accent map for one visible model set: id → { fill, custom }.
		 *  `fill` is a CSS color expression (hex, oklch(), or a color-mix()
		 *  against a theme token); `custom` marks user overrides, which the
		 *  B&W theme leaves vivid while auto colors turn grayscale. */
		function modelAccentMap(models, overrides) {
			const list = (Array.isArray(models) ? models : [])
				.map((m) => ({
					id: String(m?.model ?? m?.key ?? ""),
					tokens: (m?.input || 0) + (m?.output || 0) + (m?.cacheRead || 0) + (m?.cacheWrite || 0),
				}))
			...
			// Family size per vendor is only known after the whole sorted set
			// has been walked, so count first, then place: k of n spreads the
			// rows evenly between the two anchors and the endpoints stay pure.
			const vendorTotals = new Map();
			for (const { id } of list) {
				const vendor = vendorAccentOf(id.toLowerCase());
				if (vendor !== null) vendorTotals.set(vendor.hex, (vendorTotals.get(vendor.hex) ?? 0) + 1);
			}
			for (const { id } of list) {
				const raw = overrides?.[id];
				const custom = typeof raw === "string" && colorRe.test(raw.trim()) ? raw.trim() : null;
				if (custom !== null) {
					map.set(id, { fill: custom.toLowerCase(), custom: true });
					continue;
				}
				const key = id.toLowerCase();
				const vendor = vendorAccentOf(key);
				if (vendor !== null) {
					const k = vendorCounts.get(vendor.hex) ?? 0;
					vendorCounts.set(vendor.hex, k + 1);
					const n = vendorTotals.get(vendor.hex) ?? 1;
					const pct = n <= 1 ? 0 : Math.round((k / (n - 1)) * 100);
					const fill = pct <= 0 ? vendor.hex
						: pct >= 100 ? vendor.soft
						: `color-mix(in srgb, ${vendor.hex} ${100 - pct}%, ${vendor.soft} ${pct}%)`;
					map.set(id, { fill, custom: false });
					continue;
				}
				let hue = accentHash(key) % 360;
				for (let guard = 0; guard < 12 && claimedHues.some((h) => accentHueGap(h, hue) < 24); guard += 1) {
					hue = (hue + 137.508) % 360;
				}
				claimedHues.push(hue);
				map.set(id, { fill: `oklch(68% 0.13 ${hue.toFixed(1)})`, custom: false });
			}
			return map;
		}
		//#endregion
		...
		function BucketChart({ buckets, granularity, today, modelBuckets, accentMap, byModel, names, legendRows, onDrillChange, onDrillHourly, children, t }) {
			...
			const segsFor = (i, bucket) => {
				...
				return top.map((seg) => ({
					...seg,
					fill: seg.key === "__other__" || seg.bare === "" ? otherFill : (accentMap?.get(seg.bare)?.fill ?? otherFill),
					cls: "dp_segModel",
				}));
			};
				...
				return jsxs("div", { children: [
					jsxs("div", { ref: drillOuterRef, className: "dp_chartOuter", onClick: () => { if (pinnedSeg !== null) setPinnedSeg(null); else exitDrill(); }, children: [
						jsxs("div", { className: "dp_drillHead", children: [
							...
							jsxs("span", { className: "dp_drillTotal", children: [
								...
								bucket.key === todayKey && typeof onDrillHourly === "function" && jsx("button", {
									type: "button", className: "dp_setLink",
									onClick: (e) => { e.stopPropagation(); onDrillHourly(); },
					...
					jsxs("div", { className: "dp_legend", children: level2 !== null
						...
						: segs.map((seg) => jsxs("span", { children: [
							...
							seg.key === "__other__" || seg.bare === "" ? t("stackOther") : names.labelOf(seg.provider ?? "", seg.bare),
							jsx("span", { className: "dp_legendVal", children: fmtTokens(seg.tokens) }),
						] }, seg.key)) }),
				] });
			}
			const multiLegend = byModel
				? (Array.isArray(legendRows) ? legendRows : []).map((row) => jsxs("span", { children: [
		...
		/** X-axis labels under the bucket chart. Fixed-cadence ticks keep the
		 *  rhythm, but bursty windows leave the biggest bar between ticks, so
		 *  the peak day and the last day are always anchored too (today
		 *  already was). */
		function BucketXLabels({ buckets, granularity, today, t }) {
			const rows = Array.isArray(buckets) ? buckets : [];
			const labelStep = rows.length <= 12 ? 1 : Math.max(1, Math.ceil(rows.length / 6));
			const todayKey = bucketOf(granularity, today);
			const totalOf = (b) => (b.input || 0) + (b.cacheRead || 0) + (b.cacheWrite || 0) + (b.output || 0);
			const maxIndex = rows.reduce((m, b, i) => (totalOf(b) > totalOf(rows[m]) ? i : m), 0);
			return jsx("div", { className: "dp_xlabels", children: rows.map((bucket, i) => jsx("span", {
				...
				children: i % labelStep === 0 || i === maxIndex || i === rows.length - 1 || bucket.key === todayKey
		...
		function CostTrendPanel({ costs, today, actualByDay, accentMap, byModel, names, bucket = "day", onDrillChange, children, t }) {
			const [hover, setHover] = useState(null);
			const [drillIdx, setDrillIdx] = useState(null);
			const [seed, setSeed] = useState(null);
			const outerRef = useRef(null);
			...
			const segsFor = (row) => {
				...
				return top.map((seg) => ({
					...seg,
					fill: seg.key === "__other__" ? otherFill : (accentMap?.get(seg.bare)?.fill ?? otherFill),
					cls: "dp_segModel",
				}));
			};
			const enterDrill = (i, e) => {
			...
			const segLabel = (seg) => (!byModel
				...
				: (seg.key === "__other__" ? t("stackOther") : names.labelOf(seg.provider ?? "", seg.bare)));
			if (drillIdx !== null && rows[drillIdx] !== undefined) {
			...
			const modelLegend = [...windowCostByModel.entries()]
				.map(([mid, cost]) => {
					...
					return { label: names.labelOf(split.provider, split.model), fill: accentMap?.get(split.model)?.fill ?? otherFill, cost };
				})
				.sort((a, b) => b.cost - a.cost)
				.slice(0, 4);
			const multiLegend = byModel
				? modelLegend.map((row) => jsxs("span", { children: [
		...
		/** Full-width status bar under the card grid: official balance (left),
		 *  the current window's session/turn/tool activity (middle) and the
		 *  updated-at + refresh pair (right). With `onToggle` the whole bar
		 *  slides open the compact estimate + budget panel below it. Balance
		 *  degraded to an error line keeps the bar (and the activity segment)
		 *  alive with an inline retry. */
		function BalanceBar({ balance, series, totals, active = false, onToggle, t }) {
			const data = balance.data;
			const clickable = typeof onToggle === "function";
			const barCls = `dp_balanceBar${clickable ? " dp_sparkBtn" : ""}${active ? " dp_sparkActive" : ""}`;
			...
			const activity = totals !== undefined && jsx("span", { className: "dp_balanceActivity", children: fill(t("windowActivity"), { s: totals.sessions || 0, t: totals.turns || 0, c: totals.toolCalls || 0 }) });
			if (data === null || data.configured !== true) {
				if (balance.error !== null) return jsxs("div", { className: barCls, ...rootProps, children: [
					...
					jsx("button", { type: "button", className: "dp_miniBtn", onClick: (e) => { e.stopPropagation(); balance.refresh(); }, disabled: balance.busy, children: t("retry") }),
					activity,
				] });
				return jsxs("div", { className: barCls, ...rootProps, children: [
					jsx("i", { className: "dp_balanceDot dp_balanceDotOff" }),
					jsx("span", { className: "dp_balanceLabel", children: t("balanceTitle") }),
					activity,
				] });
			}
			if (data.ok !== true) return jsxs("div", { className: barCls, ...rootProps, children: [
				jsx("i", { className: "dp_balanceDot dp_balanceDotOff" }),
				jsx("span", { className: "dp_balanceLabel", children: t("balanceTitle") }),
			...
			/** Runway: shared helper (balance bar + chip). */
			const runway = balanceRunwayDays(data, series);
			...
			return jsxs("div", { className: barCls, ...rootProps, children: [
				...
				jsxs("span", { className: "dp_balanceTime", children: [
					...
					jsx("button", {
						type: "button", className: "dp_miniBtn", onClick: (e) => { e.stopPropagation(); balance.refresh(); },
		 ...
		 */
		function CostSpark({ days, actual, active = false, onToggle, t, grip }) {
			...
			// The priciest day wears a small value tag so the spike reads at a
			// glance instead of demanding a cursor hunt.
			const peakIdx = totals.indexOf(Math.max(...totals));
			const peakVal = peakIdx >= 0 ? totals[peakIdx] : 0;
			const peakX = Math.min(90, Math.max(10, (peakIdx / Math.max(1, n - 1)) * 100));
			...
			/** Interactive chip: with `onToggle` the spark card acts as the tab
			 *  that selects the cost-trend view in the swap container (`active`
			 *  mirrors the selection). Without it the chip stays a passive
			 *  read-only summary. */
			const clickable = typeof onToggle === "function";
			return jsxs("div", {
				className: `dp_chip dp_sparkCard${clickable ? " dp_sparkBtn" : ""}${active ? " dp_sparkActive" : ""}`,
				...
				jsxs("div", { className: "dp_cardHead", children: [
					jsx("span", { className: "dp_cardLabel", children: t("costTrend") }),
					jsx("span", { className: "dp_cardCount", children: `${fill(t("dailyCount"), { n })} · ${fill(t("costSum"), { v: `¥${fmtCost(totals.reduce((s, v) => s + v, 0))}` })}` }),
				] }),
				jsxs("div", { className: "dp_cardBody dp_sparkBody", children: [
					jsx("svg", {
						className: "dp_hourSvg", viewBox: `0 0 ${n - 1} ${H}`, preserveAspectRatio: "none",
						"aria-hidden": "true",
						children: [
							// Draw order = z-order: the billed actual line sits on top
							// of the estimate family (it is the ground truth), the
							// dashed-light estimate reads as the projection behind it.
					...
					hover !== null && jsx("span", { className: "dp_costCross", style: { left: `${hoverX * 100}%` } }),
					hover !== null && jsx("span", {
						className: "dp_costDot",
						style: { left: `${hoverX * 100}%`, top: `${y((hover.peak || 0) + (hover.offpeak || 0))}%` },
					}),
					peakVal > 0 && jsx("span", {
						className: "dp_costPeakLabel",
						style: { left: `${peakX}%`, top: `${y(peakVal)}%` },
						children: `¥${fmtCost(peakVal)}`,
					}),
					hover !== null && jsx("div", {
						className: "dp_tip",
				...
				grip === undefined || grip === null ? null : grip,
			] });
		}

		/** Height-only resize grip for the first card row. Both cards read one
		 *  shared row height, so dragging the corner moves them together; the
		 *  value is browser-local and double-click restores the derived ratio. */
		function CardRowGrip({ value, onChange, onCommit, t }) {
			const clamp = (v) => Math.max(CARD_H_MIN, Math.min(CARD_H_MAX, Math.round(v)));
			const measure = (el) => {
				const grid = el.closest(".dp_chips");
				const card = grid === null ? null : grid.querySelector(".dp_chip");
				return card === null ? 220 : card.getBoundingClientRect().height;
			};
			const current = (el) => (typeof value === "number" ? value : measure(el));
			return jsx("span", {
				className: "dp_grip",
				role: "button",
				tabIndex: 0,
				title: t("resizeCards"),
				"aria-label": t("resizeCards"),
				onClick: (e) => e.stopPropagation(),
				onDoubleClick: (e) => { e.stopPropagation(); onCommit(null); },
				onKeyDown: (e) => {
					if (e.key !== "ArrowUp" && e.key !== "ArrowDown") return;
					e.preventDefault();
					e.stopPropagation();
					const next = clamp(current(e.currentTarget) + (e.key === "ArrowUp" ? -8 : 8));
					onChange(next);
					onCommit(next);
				},
				onPointerDown: (e) => {
					...
					const startH = current(el);
					let latest = clamp(startH);
					try { el.setPointerCapture(e.pointerId); } catch (error) { /* unsupported */ }
					const move = (ev) => {
				...
				children: jsxs("svg", { width: "10", height: "10", viewBox: "0 0 10 10", "aria-hidden": "true", children: [
					jsx("path", { d: "M9 1L1 9", stroke: "currentColor", strokeWidth: "1.4", strokeLinecap: "round", fill: "none" }),
					jsx("path", { d: "M9 5.5L5.5 9", stroke: "currentColor", strokeWidth: "1.4", strokeLinecap: "round", fill: "none" }),
				] }),
			});
		}

		/** Donut gauge for the cache hit share of input-side tokens. */
		/** Cache-hit donut card. Doubles as the tab that selects the usage
		 *  trend view in the swap container (the cost spark card selects the
		 *  cost trend) — `onToggle`/`active` drive that, no arrows needed. */
		function CacheRing({ totals, active = false, onToggle, t, grip }) {
			...
			const clickable = typeof onToggle === "function";
			return jsxs("div", {
				className: `dp_chip${clickable ? " dp_sparkBtn" : ""}${active ? " dp_sparkActive" : ""}`,
				onClick: clickable ? onToggle : undefined,
				role: clickable ? "button" : undefined,
				tabIndex: clickable ? 0 : undefined,
				"aria-pressed": clickable ? active : undefined,
				...
				children: [
				jsxs("div", { className: "dp_cardHead", children: [
					jsx("span", { className: "dp_cardLabel", children: t("cacheTitle") }),
				] }),
		...
		function ModelBars({ models, modelColors, pricing = [], fx, costEnabled = false, monthly = [], names, t }) {
			const rows = (Array.isArray(models) ? models : [])
				.filter((m) => (m.input || 0) + (m.output || 0) + (m.cacheRead || 0) + (m.cacheWrite || 0) > 0)
				.slice(0, 6);
			const accents = useMemo(() => modelAccentMap(rows, modelColors), [rows, modelColors]);
			...
			return jsxs("div", { className: "dp_listBox", children: [
				...
				rows.map((row, i) => {
					...
					const label = row.model === "unknown" ? t("unknownModel") : names.labelOf(row.provider ?? "", row.model);
					return jsxs("div", { className: "dp_barRow", children: [
						jsx("span", { className: "dp_barRowName", title: row.key ?? row.model, children: label }),
						jsx("span", { className: "dp_barRowTrack", children: jsx("span", {
							className: `dp_barRowFill${accents.get(row.model)?.custom === true ? " dp_accentCustom" : ""}`,
							style: { width: `${(total / max) * 100}%`, "--dp-accent-fill": accents.get(row.model)?.fill },
						}) }),
						jsxs("span", {
		...
		function SessionDetail({ sessionId, pricing = [], fx = {}, monthly = [], costEnabled = true, model = "", catalog = null, names, t }) {
			...
			const modelFilter = model === "" || model === null || model === undefined ? null : String(model);
			const modelComposite = modelFilter !== null && modelFilter.includes(MODEL_SEP);
			const events = (Array.isArray(state.data?.events) ? state.data.events : [])
				.filter((e) => e !== null && typeof e === "object"
					&& (modelFilter === null
						|| (modelComposite ? e.key === modelFilter : (e.key === modelFilter || splitModelKey(e.key).model === modelFilter))))
				.sort((a, b) => a.t - b.t);
			...
			const segments = breaksSegments(events, breaks, { pricing, fx, monthly, model, catalog });
			...
			return jsxs("div", { className: "dp_sessDetail", children: [
				jsxs("div", { className: "dp_curveWrap", children: [
					jsx("svg", {
						ref: svgRef,
						viewBox: `0 0 ${W} ${H}`,
						preserveAspectRatio: "none",
				...
				jsxs("div", { className: "dp_breakRow", children: [
					...
					breaks.map((b) => jsx("button", {
						key: b, type: "button", className: "dp_breakChip", onClick: () => removeBreak(b),
						children: `✕ ${fmtClockMs(b)}`,
					})),
				] }),
		 ...
		 *  analysis on demand. */
		function SessionsPanel({ groups = [], costEnabled = true, pricing = [], fx = {}, monthly = [], model = "", catalog = null, names, t }) {
				...
				return jsxs("div", { className: "dp_sessGroup", children: [
					...
					expanded && jsxs("div", { className: "dp_sessBody", children: [
						group.sessions.map((s) => {
							...
							return jsxs("div", { className: "dp_sessRow", children: [
								jsx("span", { className: s.subagent ? "dp_sessBadge dp_sessBadgeSub" : "dp_sessBadge", children: s.subagent ? `${t("sessionSub")}${s.delegationDepth > 0 ? " " + s.delegationDepth : ""}` : t("sessionMain") }),
								jsx("span", { className: "dp_sessId", title: s.id, children: s.id === null || s.id === undefined ? "—" : String(s.id).slice(-8) }),
								...
								jsx("button", { type: "button", className: "dp_miniBtn", onClick: () => setDetail(detailOpen ? null : s.id), children: t("sessionOpenDetail") }),
								jsx("div", { className: "dp_sessModels", children: (Array.isArray(s.modelRows) ? s.modelRows : []).map(modelRow) }),
								detailOpen && jsx(SessionDetail, { key: s.id, sessionId: s.id, pricing, fx, monthly, costEnabled, model, catalog, names, t }),
							] }, s.id ?? i);
						}),
					] }),
				] }, name);
		...
		function Toolbar({ rangeKey, setRangeKey, custom, setCustom, project, setProject, knownProjects, model, setModel, knownModels, names, t, filterSlot }) {
			...
			/** Model picker options carry composite keys (`provider\u0000model`,
			 *  bare id for provider-less rows); labels come from the catalog
			 *  names helper. */
			const modelLabel = (key) => {
				const { provider, model } = splitModelKey(key);
				return model === "unknown" ? t("unknownModel") : names.labelOf(provider, model);
			};
			const modelOptions = useMemo(() => [
				{ value: "", label: t("modelAll") },
				...knownModels.map((key) => ({ value: key, label: modelLabel(key) })),
			], [knownModels, names, t]);
		...
		function exportDailyCsv({ t, view, costDays, balanceSeries = [], fromDay, toDay }) {
			...
			const rows = [];
			let sumSessions = 0, sumIn = 0, sumRead = 0, sumWrite = 0, sumOut = 0, sumCost = 0;
			for (const bucket of buckets) {
				...
				rows.push([
					...
					spend === undefined || spend?.spend === null ? "" : spend.spend,
				]);
			}
			const header = [t("expDate"), t("expSessions"), t("expInput"), t("expCacheRead"),
				t("expCacheWrite"), t("expOutput"), t("expTotal"), t("expHitRate"), t("expCost"), t("expSpend")];
			const grand = [t("expSum"), sumSessions, sumIn, sumRead, sumWrite, sumOut,
				sumIn + sumRead + sumWrite + sumOut, "", Math.round(sumCost * 1e6) / 1e6, ""];
		 ...
		 * dashboard down.
		 */
		function PulseDashboard({ t, headerExtra, onConfigure, floatActions = false, renderSlot: renderPanelsSlot, actions }) {
			const [panels] = useState(loadPanels);
			/** Row-one card height override; `null` keeps the derived ratio. */
			const [cardH, setCardH] = useState(loadCardH);
			...
			/** 费用走势聚焦态：点击 spark 卡片后，费用走势大面板在用量走势的
			 *  槽位向下展开替换它，其余面板模糊压暗；再点一次或 Esc 还原。 */
			/** 切换容器的两个页签：缓存命中卡 = 用量走势，费用走势卡 = 费用
			 *  走势。纯 tab 切换，无聚焦/压暗机制。 */
			const [swapTab, setSwapTab] = useState("use");
			/** 状态条点击后向下平移展开的 费用估算 + 月度预算 合并卡。 */
			...
			const accentMap = useMemo(
				() => modelAccentMap(view === null ? [] : view.models, data === null ? {} : data.modelColors),
				[view, data],
			);
			...
			const modelLegendRows = byModel && view !== null
				? (Array.isArray(view.models) ? view.models : []).slice(0, 4).map((m) => ({
					fill: accentMap.get(m.model)?.fill ?? "var(--dsw-alias-label-tertiary)",
					label: names.labelOf(m.provider ?? "", m.model),
				}))
				: null;
			...
			const sessionGroupsData = useMemo(
				() => (data === null ? [] : sessionGroups(data.sessions, { pricing: data.pricing, fx: data.fx, monthly: data.monthly, model, catalog })),
				[data, model, catalog],
			);
			...
			const exportBtn = jsx("button", {
				...
				children: jsx("svg", { width: 15, height: 15, viewBox: "0 0 16 16", fill: "none", "aria-hidden": true, children: [
					jsx("path", { d: "M8 2.2v7.3M8 9.5 4.9 6.4M8 9.5l3.1-3.1", stroke: "currentColor", strokeWidth: 1.4, strokeLinecap: "round", strokeLinejoin: "round" }),
					jsx("path", { d: "M2.9 10.6v2.5h10.2v-2.5", stroke: "currentColor", strokeWidth: 1.4, strokeLinecap: "round" }),
				] }),
			});
			...
			const header = jsxs("div", { className: `dp_headerRow${floatActions ? " dp_headerRowFloat" : ""}`, children: [
				...
				jsx("span", { className: "dp_buildTag", title: `dsh-pulse ${DP_BUILD}`, children: DP_BUILD }),
			...
			if (data === null || stats.status === "loading" || staleWindow) {
				return jsxs("div", { className: "dp_root", "data-dp-theme": theme, children: [
					...
					view !== null
						? jsx(Toolbar, { rangeKey, setRangeKey, custom, setCustom, project, setProject, knownProjects: view.knownProjects, model, setModel, knownModels: view.knownModels, names, t, filterSlot: renderPanelsSlot })
			...
			if ((data.sessions ?? []).length === 0) {
				return jsxs("div", { className: "dp_root", "data-dp-theme": theme, children: [
					...
					jsxs("div", { className: "dp_stateBox", children: [
						...
						jsx("span", { className: "dp_stateBody", children: fill(t("windowEmptyBody"), { n: daysBetween(data.fromDay, data.toDay) }) }),
					] }),
				] });
			}
			if (!view.hasData) {
			...
			/** 选中第三方供应商（非 DeepSeek 官方 `deepseek-official`）的模型时，
			 *  官方余额与这个账号无关，余额面板（含对账线）隐藏；若该模型没有
			 *  任何定价规则覆盖（供应商精确或通配），费用估算与月度预算一并
			 *  隐藏。官方或无法归类的（无供应商）模型不受影响。 */
			const modelSel = model === "" ? null : splitModelKey(model);
			const thirdParty = modelSel !== null && modelSel.provider !== "" && modelSel.provider !== "deepseek-official";
			const modelConfigured = modelSel !== null && (
				(Array.isArray(data.monthly) ? data.monthly : []).includes(modelSel.provider)
				|| (Array.isArray(data.pricing) ? data.pricing : []).some((rule) => {
					if (typeof rule?.model !== "string" || rule.model !== modelSel.model) return false;
					const provider = typeof rule.provider === "string" && rule.provider.length > 0 ? rule.provider : "";
					return provider === "" || provider === modelSel.provider;
				})
			);
			const hideCostish = thirdParty && !modelConfigured;
			const bucketCount = view.buckets.length;
			/** Cost-trend tab availability: needs the swap slot, a multi-day
			 *  window, configured pricing and the spark card's own gates. When
			 *  any gate drops (e.g. the range narrows to today) the usage
			 *  trend quietly takes the container back. */
			const costFocusable = panels.trend !== false && panels.costTrend !== false
				&& costEnabled && !hideCostish && !hourly
				&& view !== null && view.cost.configured === true && costDays !== null;
			const swapView = swapTab === "cost" && costFocusable ? "cost" : "use";
			...
			const costRowCount = view !== null ? costPeriodRows(view.bucketCosts, costBucket).length : 0;
			/** Status bar + its slide-down panel. The bar renders when balance
			 *  info or the activity strip is welcome; the panel needs at least
			 *  one of estimate / budget. `barResolved`: the balance endpoint
			 *  answered meaningfully (ok payload, or a failure worth showing
			 *  with retry) — an unconfigured feature stays silent. */
			const barResolved = balance.error !== null
				|| (balance.data !== null && balance.data.configured === true);
			const estOn = panels.cost !== false && costEnabled && !hideCostish;
			const budgetOn = panels.budget !== false && costEnabled && !hideCostish;
			const barOpenable = estOn || budgetOn;
			const showChips = panels.cache !== false || panels.balance !== false || panels.chips !== false || estOn;
			/** Which row-one cards are actually mounted, so the height grip
			 *  lands on the last visible one. */
			const cacheOn = panels.cache !== false;
			const costOn = panels.cost !== false && cost.configured === true && costDays !== null
				&& costDays.some((d) => (d.peak || 0) + (d.offpeak || 0) > 0);
			const rowGrip = (cacheOn || costOn)
				? jsx(CardRowGrip, {
					value: cardH,
					onChange: setCardH,
					onCommit: (next) => { setCardH(next); saveCardH(next); },
					t,
				})
				: null;
			return jsxs("div", { className: "dp_root", "data-dp-theme": theme, children: [
				...
				showChips && jsxs("div", {
					className: "dp_chips",
					"data-dp-card-h": cardH === null ? undefined : "",
					style: cardH === null ? undefined : { "--dp-card-h": `${cardH}px` },
					children: [
					cacheOn && jsx(CacheRing, {
						totals,
						active: swapView === "use",
						onToggle: panels.trend !== false ? () => setSwapTab("use") : undefined,
						t,
						grip: costOn ? null : rowGrip,
					}),
					costOn
						&& jsx(CostSpark, {
							days: costDays,
							actual: thirdParty ? null : (Array.isArray(data?.balanceSeries) ? data.balanceSeries : null),
							active: swapView === "cost",
							onToggle: costFocusable ? () => setSwapTab("cost") : undefined,
							t,
							grip: rowGrip,
						}),
					(panels.balance !== false || panels.chips !== false) && jsx(BalanceBar, {
						balance,
						series: Array.isArray(data?.balanceSeries) ? data.balanceSeries : null,
						totals: panels.chips === false ? undefined : totals,
						active: barOpen,
						onToggle: barOpenable ? () => setBarOpen((v) => !v) : undefined,
						t,
					}),
					// Extension chips (`pulse.dashboard.panel`'s sibling for KPI
					// tiles): same render-site props as the panel slot.
					typeof renderPanelsSlot === "function" && renderPanelsSlot("pulse.dashboard.chip", { data, view, busy: stats.busy }),
				] }),
				/** 状态条点击 → 向下平移展开「费用估算 + 月度预算」合并卡：
				 *  信息量小，合并为一行两列、收窄高度，不与走势图争位。 */
				barOpen && barOpenable && jsxs("div", { className: "dp_estBudgetPanel", children: [
				...
				/** 切换容器（两态 tab）：用量走势（默认常驻，缓存命中卡选中）
				 *  ⇄ 费用走势（费用走势卡选中）。视图等高（256px），key 切换
				 *  重放轻量过渡；费用走势底部附官网口径小字。 */
				panels.trend !== false && jsxs("div", { children: [
					jsxs("div", { key: swapView, className: "dp_swapIn", children: [
						jsxs("div", { className: `dp_panelTitle${swapView === "cost" || (swapView === "use" && !hourly && !heatmap && hasModelSplit) ? " dp_hasStack" : ""}`, children: [
							jsx("span", { children: swapView === "cost" ? t("costTrend") : t("trendTitle") }),
							jsx("span", { className: "dp_panelCount", children: swapView === "cost"
								? (costBucket === "week" ? fill(t("weekCount"), { n: costRowCount }) : costBucket === "month" ? fill(t("monthCount"), { n: costRowCount }) : fill(t("dailyCount"), { n: bucketCount }))
								: hourly ? t("hourCount") : fill(t("dailyCount"), { n: bucketCount }) }),
							swapView === "cost" ? jsxs("div", { className: "dp_seg dp_stackToggle", children: [
								jsx("button", { type: "button", className: `dp_segBtn${costStack !== "model" ? " dp_segBtnActive" : ""}`, onClick: () => applyCostStack("tiers"), children: t("stackTiers") }),
								jsx("button", { type: "button", className: `dp_segBtn${costStack === "model" ? " dp_segBtnActive" : ""}`, onClick: () => applyCostStack("model"), children: t("stackModel") }),
							] }) : (!hourly && !heatmap && hasModelSplit && jsxs("div", { className: "dp_seg dp_stackToggle", children: [
								jsx("button", { type: "button", className: `dp_segBtn${!byModel ? " dp_segBtnActive" : ""}`, onClick: () => applyTrendStack("type"), children: t("stackType") }),
								jsx("button", { type: "button", className: `dp_segBtn${byModel ? " dp_segBtnActive" : ""}`, onClick: () => applyTrendStack("model"), children: t("stackModel") }),
							] })),
						] }),
						jsx("div", { className: "dp_trendBody", children: swapView === "cost"
							...
							: hourly
								...
								: heatmap
									...
									: jsx(BucketChart, { buckets: view.buckets, granularity: "day", today: data.today, modelBuckets: view.modelBuckets, accentMap, byModel, names, legendRows: modelLegendRows, onDrillChange: setTrendDrilled, onDrillHourly: () => setRangeKey("1"), children: !trendFocused && jsx(BucketXLabels, { buckets: view.buckets, granularity: "day", today: data.today, t }), t }) }),
					] }),
				] }),
				/** 模型分布：全宽汇总节，紧随切换容器（当前窗口的静态汇总）。 */
				panels.models !== false && jsx(ModelBars, { models: view.models, modelColors: data.modelColors, pricing: data.pricing, fx: data.fx, costEnabled, monthly: data.monthly, names, t }),
				panels.projects !== false && project === "" && jsxs("div", { children: [
					jsx("div", { className: "dp_panelTitle", children: t("projectsTitle") }),
					jsx(ProjectTable, { projects: view.projects, topProjects: data.topProjects, onSelect: setProject, t }),
				] }),
				panels.sessions !== false && project !== "" && jsxs("div", { children: [
					jsxs("div", { className: "dp_panelTitle", children: [
						jsx("span", { children: t("projectDetailTitle") }),
						jsx("button", { type: "button", className: "dp_setLink", style: { marginLeft: "auto" }, onClick: () => setProject(""), children: `← ${t("back")}` }),
					] }),
					jsx(SessionsPanel, {
						groups: sessionGroupsData.filter((g) => g.project === project), costEnabled, pricing: data.pricing, fx: data.fx,
						monthly: data.monthly, model, catalog, names, t,
					}),
				] }),
				...
				typeof renderPanelsSlot === "function" && renderPanelsSlot("pulse.dashboard.panel", { data, view, busy: stats.busy }),
			] });
		}
		//#endregion
		...
		function buildEditorRows(data) {
			...
			for (const groupEntry of Array.isArray(data.catalog) ? data.catalog : []) {
				...
				for (const model of Array.isArray(groupEntry.models) ? groupEntry.models : []) {
					if (typeof model?.id !== "string" || model.id === "") continue;
					const rule = ruleFor(provider, model.id) ?? {};
					rows.push({
						key: modelKey(provider, model.id),
						provider, model: model.id,
						name: model.name ?? "",
						group: groupEntry.displayName ?? groupEntry.provider ?? "",
						origin: "catalog",
		...
		function PricingPage({ t }) {
			const [state, setState] = useState({
				...
				rows: [], official: [], monthly: [], modelColors: {}, pricingDirty: false, writable: false, hasCatalog: false,
			...
			const save = () => {
				...
				const payload = { costEnabled: state.costEnabled === true, monthly: state.monthly };
			...
			const accentPreview = modelAccentMap(accentRows.map((row) => ({ model: row.model, tokens: 0 })), state.modelColors);
			...
			state.rows.forEach((row, i) => {
				if (row.provider !== lastProvider) {
					...
					rendered.push(jsxs("div", { className: "dp_setGroup", children: [
						...
						jsx("button", {
							type: "button",
							className: `dp_miniBtn dp_setMonthlyBtn${monthlyOn ? " dp_miniBtnOn" : ""}`,
							disabled: !state.writable || state.saving,
			...
			if (state.status === "loading") {
				...
				body = jsxs("div", { className: "dp_setGrid", children: [
					...
					jsxs("div", { className: "dp_accentCard", children: [
						...
						accentRows.map((row) => {
							const accent = accentPreview.get(row.model);
		...
		function ComparePage({ t }) {
			const [settings, setSettings] = useState({ status: "loading", pricing: [], catalog: [], monthly: [], fx: null, currency: "CNY", error: null });
			...
			}, [settings.pricing, settings.catalog, settings.currency, settings.monthly, state.manuals, state.tiers, names]);
		...
		function PanelsPage({ t, theme, setTheme }) {
			...
			return jsxs("div", { className: "dp_setGrid", children: [
				...
				jsxs("div", { className: "dp_setPanel", children: [
					...
					jsxs("div", { className: "dp_seg", children: THEMES.map((key) => {
						const label = key === "blue" ? t("themeBlue")
							: key === "pink" ? t("themePink")
								: key === "orange" ? t("themeOrange") : t("themeBw");
						return jsx("button", {
							type: "button",
							className: `dp_segBtn${theme === key ? " dp_segBtnActive" : ""}`,
		...
		function PulseCommandCard({ t, renderFactorySlot }) {
			return jsx("div", { className: "dp_card dp_variantCard", children:
				typeof renderFactorySlot === "function"
					? renderFactorySlot("pulse.dashboard", {}, { fallback: jsx(PulseDashboard, { t }) })
					: jsx(PulseDashboard, { t })
			});
		}
		...
		function PulseOverlay({ t, renderFactorySlot }) {
			...
			const refreshBtn = jsx("button", {
				type: "button", className: "dp_iconBtn", onClick: () => loadStats(stats.from, stats.to),
			...
			return jsx("div", {
				...
				children: jsx("div", { className: "dp_overlayCard", role: "dialog", "aria-label": t("title"), children: [
					jsx("div", { className: "dp_overlayScroll", children: dashboard }),
					jsxs("div", { className: "dp_overlayActions", children: [refreshBtn, closeBtn] }),
				] }),
			});
		}
		//#endregion
		...
		function FootBalanceRow({ t, setFootBalance, useStore }) {
			...
			return jsxs("div", { className: "dp_setRow", children: [
				...
				typeof Switch === "function"
					...
					: jsx("input", {
						type: "checkbox", className: "dp_setSwitchBox", checked: enabled,
						"aria-label": t("generalFootBalance"), onChange: (e) => change(e.target.checked),
					}),
			] });
		}
		...
		function PulseDetailBadge({ t }) {
			return jsx("span", { className: "dp_sessBadge", children: t("detailBadge") });
		}

		/** Section under the bundle detail page's own content. */
		function PulseDetailSection({ t }) {
