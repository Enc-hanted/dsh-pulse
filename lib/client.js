window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 *  one family, stepped softer by mixing toward the tertiary label
		 *  token, so the heaviest model wears the pure brand color and the
		 *  step adapts to light/dark themes for free. */
		...
		const VENDOR_ACCENTS = [
			{ keys: ["deepseek"], hex: "#4d6bfe" },
			{ keys: ["claude", "anthropic"], hex: "#d97757" },
			{ keys: ["openai", "chatgpt", "gpt", "davinci", "o1", "o3", "o4"], hex: "#10a37f" },
			{ keys: ["qwen", "qwq", "tongyi"], hex: "#847ace" },
			{ keys: ["glm", "zhipu", "chatglm"], hex: "#2f6bff" },
			{ keys: ["minimax", "abab"], hex: "#ff3763" },
			{ keys: ["grok", "xai"], hex: "#6e6d66" },
			{ keys: ["mimo", "xiaomi"], hex: "#ff6900" },
			{ keys: ["llama", "meta"], hex: "#0668e1" },
			{ keys: ["gemini", "google", "gemma"], hex: "#1c69ff" },
			{ keys: ["mistral", "mixtral", "codestral", "magistral"], hex: "#ff7000" },
			{ keys: ["kimi", "moonshot"], hex: "#4b4b52" },
			{ keys: ["hunyuan"], hex: "#0052d9" },
			{ keys: ["doubao", "skylark"], hex: "#4d53e8" },
		];
		...
		function modelAccentMap(models, overrides) {
			...
			for (const { id } of list) {
				...
				if (vendor !== null) {
					...
					const purity = 100 - Math.min(36, k * 12);
					map.set(id, { fill: `color-mix(in srgb, ${vendor.hex} ${purity}%, var(--dsw-alias-label-tertiary))`, custom: false });
					continue;
				}
