window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function ModelBars({ models, pricing = [], fx, costEnabled = false, monthly = [], names, t }) {
		...
		function PulseDashboard({ t, headerExtra, onConfigure, floatActions = false, renderSlot: renderPanelsSlot, actions }) {
			...
			return jsxs("div", { className: "dp_root", "data-dp-theme": theme, children: [
				...
				(panels.cache !== false || panels.models !== false) && jsxs("div", { className: "dp_twoCol", children: [
					...
					panels.models !== false && jsx(ModelBars, { models: view.models, pricing: data.pricing, fx: data.fx, costEnabled, monthly: data.monthly, names, t }),
				] }),
		...
		function PricingPage({ t }) {
			const [state, setState] = useState({
				...
				rows: [], official: [], monthly: [], pricingDirty: false, writable: false, hasCatalog: false,
				...
				fetch("/pulse/settings", {
					...
					.then(async (res) => {
						...
						setState((s) => ({
							...
							pricingDirty: false,
							// The edited rows are persisted now — clear the dirty
							// flags so a re-save does not rewrite them.
							rows: s.rows.map((row) => ({ ...row, dirty: false })),
						}));
